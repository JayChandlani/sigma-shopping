...
const {cartProduct,setCartProduct,status}=useContext(AuthContext);
    const productId = useParams()
    const [product, setProduct] = useState({});
    useEffect(() => {
        setProduct(productData.find((items) => {
            return items.id === Number(productId.id)
        })
        );
    }, [productId])
                        ...
                        <div className="row">
                            <div className="col-md-6">
                                <div className="images p-3">
                                    <div className="text-center p-4"> <img alt='img' id="main-image" src={product.image} width="250" /> </div>
                                    <div className="thumbnail text-center"> <img alt='img' src={product.image} width="70" /> <img alt='img' src={product.image} width="70" /> </div>
                                </div>
                            </div>
                            <div className="col-md-6">
                                <div className="product p-4">
                                    ...
                                    <div className="mt-4 mb-3"> <span className="text-uppercase text-muted brand">{product.name}</span>
                                        ...
                                        <div className="price d-flex flex-row align-items-center"> <span className="act-price">{product.price}</span>
                                            <div className="ml-2"> <small className="dis-price">{product.price}</small> <span>40% OFF</span> </div>
                                        </div>
                                    </div>
                                    ...
                                    <div  className="cart mt-4 align-items-center"> <button onClick={()=>status?handleCart(product):notify()} className="btn btn-danger text-uppercase mr-2 px-4">Add to cart</button> <i className="fa fa-heart text-muted"></i> <i className="fa fa-share-alt text-muted"></i> </div>
                                </div>
                            </div>
                        </div>
                    </div>
